codeunit 60010 "Release MaterialIssue Document"
{
            ...
            IF user.FINDFIRST THEN BEGIN
                IF user.Dept <> 'STR' THEN
                    IF user.levels = FALSE THEN BEGIN
                        IF NOT ((Rec."Transfer-from Code" = 'CAL') AND (USERID = 'EFFTRONICS\VIJAYALAKSHMIB') AND (USERID = 'EFFTRONICS\SUVARCHALADEVI')) THEN
                            ERROR('YOU HAVE NO PERMISSION TO AUTHORIZE');
                    END;
            END;
        END;
    ...
    begin
        IF USERID IN ['EFFTRONICS\ANILKUMAR', 'EFFTRONICS\MARY', 'EFFTRONICS\ANANDA', 'EFFTRONICS\RATNA', 'EFFTRONICS\DMADHAVI', 'EFFTRONICS\PADMASRI',
                      ...
                      'EFFTRONICS\GRAVI', 'EFFTRONICS\VISHNUPRIYA', 'EFFTRONICS\PKOTESWARARAO', 'EFFTRONICS\DURGAMALLESWARI', 'EFFTRONICS\TLAKSHMI', 'EFFTRONICS\TULASI', 'EFFTRONICS\RENUKACH', 'EFFTRONICS\RRAHUL', 'EFFTRONICS\B2BOTS', 'EFFTRONICS\SUVARCHALADEVI', 'EFFTRONICS\TPRIYANKA'] THEN BEGIN
    ...
    begin
        IF MIH.Status <> MIH.Status::Released THEN BEGIN
            IF USERID IN ['EFFTRONICS\ANILKUMAR', 'EFFTRONICS\ANANDA', 'EFFTRONICS\MARY', 'EFFTRONICS\RATNA', 'EFFTRONICS\PADMASRI', 'EFFTRONICS\SUJANI',
                          'EFFTRONICS\PADMA', 'EFFTRONICS\SIRISHA', 'EFFTRONICS\VIJAYALAKSHMIB', 'EFFTRONICS\RENUKAH'] THEN BEGIN
