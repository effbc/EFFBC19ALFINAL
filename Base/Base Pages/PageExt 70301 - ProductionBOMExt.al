pageextension 70301 ProductionBOMExt extends 99000786
{
    layout
    {
        modify(Status)
        {
            ...
            begin
                IF (COPYSTR(Rec."No.", 1, 8) <> 'ECMPBPCB') AND (Rec.Status = Rec.Status::Certified) AND NOT (USERID IN ['EFFTRONICS\JHANSI', 'EFFTRONICS\ANILKUMAR', 'ERPSERVER\ADMINISTRATOR', 'EFFTRONICS\SUVARCHALADEVI', 'EFFTRONICS\MANOJA', 'EFFTRONICS\RAMALAKSHMI', 'EFFTRONICS\NVSIVAKUMARI']) THEN
                    ERROR('You dont have permissions to Certify the BOM');

                IF (COPYSTR(Rec."No.", 1, 8) = 'ECMPBPCB') AND NOT (USERID IN ['EFFTRONICS\JHANSI', 'EFFTRONICS\RSILPARANI', 'EFFTRONICS\RATNARAVALI', 'EFFTRONICS\UBEDULLA', 'EFFTRONICS\ANILKUMAR', 'ERPSERVER\ADMINISTRATOR', 'EFFTRONICS\MANOJA', 'EFFTRONICS\SUVARCHALADEVI', 'EFFTRONICS\RAMALAKSHMI', 'EFFTRONICS\NVSIVAKUMARI']) THEN
                ...
                IF ((Rec.Status = Rec.Status::"Under Development") OR (Rec.Status = Rec.Status::New)) AND (xRec.Status = xRec.Status::Certified) AND NOT (USERID IN ['EFFTRONICS\VANIDEVI', 'ERPSERVER\ADMINISTRATOR', 'EFFTRONICS\SUVARCHALADEVI', 'EFFTRONICS\MANOJA', 'EFFTRONICS\RAMALAKSHMI', 'EFFTRONICS\NVSIVAKUMARI']) THEN
