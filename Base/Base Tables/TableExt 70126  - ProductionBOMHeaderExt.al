    ...
    fields
    {
        ...
        modify(Status)
        {
            ...
            begin
                ...
                if (Rec."Creation Date" <= 20200909D) and (xRec.Status.AsInteger() = 1) and not (UserId in ['EFFTRONICS\VANIDEVI', 'EFFTRONICS\VISHNUPRIYA']) then
                ...
                if (CopyStr("No.", 1, 8) = 'ECMPBPCB') and not (UserId in ['EFFTRONICS\PARVATHI', 'EFFTRONICS\JHANSI', 'EFFTRONICS\RSILPARANI', 'EFFTRONICS\RATNARAVALI', 'EFFTRONICS\UBEDULLA', 'EFFTRONICS\VISHNUPRIYA', 'EFFTRONICS\ANILKUMAR']) then
                    Error('You dont have permissions to Certify the MASTER BOM');


                if (CopyStr("No.", 1, 8) <> 'ECMPBPCB') and (Status = Status::Certified) and not (UserId in ['EFFTRONICS\JHANSI', 'EFFTRONICS\VANIDEVI', 'EFFTRONICS\ANILKUMAR', 'EFFTRONICS\VISHNUPRIYA']) then
                ...
                if ((Status <> Status::Certified) and (Status <> Status::Closed) and (Status = Status::New) and
                     (UserId <> 'SUPER') and (UserId <> 'EFFTRONICS\JHANSI') and (UserId <> 'EFFTRONICS\DIR') and (UserId <> 'EFFTRONICS\PARVATHI') and (UserId <> 'EFFTRONICS\RATNARAVALI') and (UserId <> 'EFFTRONICS\RSILPARANI')
                     and (UserId <> 'EFFTRONICS\UBEDULLA') and ("Stranded BOM" <> true)
                     and (UserId <> 'EFFTRONICS\SWATHI')) then begin
