   ...
   ItemJournalLine."Order Line No.", ItemJournalLine."Inspectin Receipt No.", ItemJournalLine."Prod. Order Comp. Line No.") THEN BEGIN
    ...
    begin
        ...
        IF ("to mail" <> '') THEN
            ...
            EmailMessage.Create("to mail", Mail_Subject, Mail_Body);
    ...
    local procedure OnAfterReleaseATOscust(var SalesHeader: Record "Sales Header"; var SalesLine: Record "Sales Line"; PreviewMode: Boolean)
    var
        ErrorMess: text[1000];
        SHA: Record 5107;
    ...
    begin
        ...
        IF SalesHeader."Document Type" = SalesHeader."Document Type"::Order THEN BEGIN                                                     //Mail Code for Release Condition
            ...
            IF (SalesHeader."Sell-to Customer No." <> 'CUST00822') AND (SalesHeader."Sell-to Customer No." <> 'CUST00536') AND (COPYSTR(SalesHeader."No.", 15, 1) <> 'L') AND (COPYSTR(SalesHeader."No.", 15, 1) <> 'T') THEN BEGIN
                ...
                IF SHA.FINDFIRST THEN BEGIN
                    ...
                    IF CLE1.FINDFIRST THEN BEGIN
                        ...
                        REPORT.SAVEASPDF(50026, '\\erpserver\ErpAttachments\Changelog.PDF', CLE2);
                ...
                ELSE BEGIN
                    ...
                    "to mail" := 'mk@effe.in,erp@efftronics.com,padmaja@efftronics.com,rajani@efftronics.com,purchase@efftronics.com,'; //,rajani@efftronics.com
                    "to mail" += 'controlroom@efftronics.com,prasanthi@efftronics.com,'; //,pmurali@efftronics.com
                                                                                         //"to mail"+='ganuradha@efftronics.com,';
                                                                                         //"to mail"+='vanidevi@efftronics.com,';
                    "to mail" += 'sales@efftronics.com,';
                    "to mail" += 'marketing@efftronics.com,anvesh@efftronics.com,';
                    "to mail" += 'cuspm@efftronics.com'; //qms@efftronics.com,
                    ...
                    IF Verticals.FINDFIRST THEN BEGIN
                        IF Verticals."Vertical Head E_mail" <> '' THEN
                            "to mail" += ',' + Verticals."Vertical Head E_mail";
                        IF Verticals."PM E_mail" <> '' THEN
                            "to mail" += ',' + Verticals."PM E_mail";
                        IF Verticals."APM E_mail" <> '' THEN
                            "to mail" += ',' + Verticals."APM E_mail";
                    END;
                    ...
                    /* {  Mail_Body+= 'Sale Order No          : '+"No.";
                      ...
                      // Mail_Body+= 'Sale Order Value(RS)   : '+FORMAT(ROUND("Sale Order Total Amount",1)); // Commented by rakesh
                       ...
                       IF SL.FINDSET THEN
                       REPEAT
                         IF (SL."Schedule Type"=0) OR (SL."Schedule No"=0) THEN
                         BEGIN
                    ...
                    if ("to mail" <> '') then
                        Mail_To.Add("to mail");
                    EmailMessage.Create(Mail_To, 'ERP - ' + Mail_Subject, Body, TRUE);
                    ...
                    IF BI_Status = TRUE THEN
                        Mail_To.Add('blockinstrument@efftronics.com');
                    ...
                    Email.Send(EmailMessage, Enum::"Email Scenario"::Default); //sujani  on 01Apr2019
                                                                               //  END;
                                                                               //********************************************* MAIL CODE END***************************************
            ...
            ELSE BEGIN
                ...
                IF SHA.FINDFIRST THEN BEGIN
                    ...
                    IF CLE1.FINDFIRST THEN BEGIN
                        ...
                        SMTP_MAIL.CreateMessage('ERP', Mail_From, Mail_To, 'ERP - ' + Subject, Body, TRUE);
                ...
                ELSE BEGIN
                    ...
                    "to mail" := 'mk@effe.in,bala@efftronics.com,padmaja@efftronics.com,marketing@efftronics.com,bharat@efftronics.com,';
                    "to mail" += 'erp@efftronics.com,qms@efftronics.com,somu@efftronics.com,controlroom@efftronics.com,vkrishna@efftronics.com';
                    "to mail" += ',purchase@efftronics.com';
                    ...
                    Mail_Subject := 'ERP- ' + 'Sale Order ' + SalesHeader."No." + ' Released on for the customer ' + SalesHeader."Sell-to Customer Name";
                    Subject := Mail_Subject;
                    EmailMessage.Create(Mail_To, 'ERP - ' + Subject, Body, TRUE);
        ...
        IF (SalesHeader."Document Type" = SalesHeader."Document Type"::Amc) THEN BEGIN                                                     //Mail Code for Release Condition for AMC Order
            SHA.SETRANGE(SHA."No.", SalesHeader."No.");
            ...
            ELSE BEGIN
                ...
                IF "Mail-Id".FINDFIRST THEN
                    // "from Mail" := "Mail-Id".MailID;
                    "to mail" := 'mk@effe.in,cuspm@efftronics.com,rajani@efftronics.com,';
                //"to mail"+='pmurali@efftronics.com,';
                "to mail" += 'sales@efftronics.com,';
                "to mail" += 'prasanthi@efftronics.com,yesu@efftronics.com,rishianvesh@efftronics.com,erp@efftronics.com';
            ...
            ELSE BEGIN
                ...
                IF (USERID <> 'EFFTRONICS\PADMAJA') THEN BEGIN
                    ...
                    ELSE BEGIN
                        ...
                        IF (COPYSTR(SalesHeader."No.", 15, 1) <> 'L') THEN BEGIN
                            "to mail" := 'padmaja@efftronics.com,prasanthi@efftronics.com,controlroom@efftronics.com,';
                            "to mail" += 'erp@efftronics.com,';
                            "to mail" += 'purchase@efftronics.com,qms@efftronics.com,marketing@efftronics.com,sales@efftronics.com';
                        END
                        ELSE BEGIN
                            "to mail" := 'mk@effe.in,bala@efftronics.com,padmaja@efftronics.com,bharat@efftronics.com,';
                            "to mail" += 'erp@efftronics.com,qms@efftronics.com,somu@efftronics.com,controlroom@efftronics.com';
                        END;
                        if ("to mail" <> '') then
                            Mail_To.Add("to mail");
                        // end
